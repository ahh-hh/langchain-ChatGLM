...
# 按格式获取指定日期字符串
# days 0表示今天 -1表示昨天 以此类推
# format 表示日期格式化规则
def get_time_str(days=0, hours=0, format='%Y-%m-%d'):
    return (datetime.datetime.now()+datetime.timedelta(days=days, hours=hours)).strftime(format)
...
def custom_log(first_msg, second_msg=""):
    ...
    file = r'' + FOLDER_LOG + '/' + get_time_str() + '.txt'
